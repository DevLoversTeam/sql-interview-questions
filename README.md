...
<details>
<summary>101. Що таке SQL dump і як його використовують?</summary>

#### SQL

`SQL dump` - це файл із SQL-інструкціями (переважно `CREATE`, `INSERT`, інколи `ALTER`), який зберігає структуру БД і/або дані для подальшого відновлення чи перенесення.

Типові сценарії використання:

- резервне копіювання (backup);
- відновлення після збою;
- міграція між середовищами (dev/stage/prod);
- перенесення даних між серверами;
- створення тестових копій бази.

Що зазвичай може містити dump:

- тільки схему (`schema-only`);
- тільки дані (`data-only`);
- схему + дані (повний dump).

Базова ідея відновлення - виконати SQL-файл у цільовій БД.

```sql
-- приклад фрагмента dump-файлу
CREATE TABLE users (
  id BIGINT PRIMARY KEY,
  email VARCHAR(255) NOT NULL UNIQUE
);
...
- регулярно перевіряти не тільки створення backup, а й фактичний restore;
- перед імпортом у production перевіряти сумісність версій СУБД;
- чутливі дані в dump-файлах шифрувати та обмежувати доступ;
- для великих БД планувати вікно обслуговування або використовувати поетапний імпорт.

**Коротко:**

- SQL dump - це SQL-файл для експорту/відновлення структури та даних БД.
- Основні кейси: backup, restore, міграція між середовищами.
- Критично важливо регулярно тестувати процес відновлення.
